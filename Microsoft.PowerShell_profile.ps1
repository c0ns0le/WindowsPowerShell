...
function Test-RDP { param(
    [Alias("hn","server","hostname","cn")]
    [parameter(Position=0)]$ComputerName,
    [parameter(Position=1)]$Port = "3389")

begin {
$script:g_ServersRDPTrue = @()
$script:g_ServersRDPFalse = @()
Write-Host "Checking $($ComputerName):$($Port) ... " -NoNewline }
process {
    if (( Test-Port $ComputerName $Port ).Open -eq $true ) { $g_ServersRDPTrue += $ComputerName ; Write-Host $true ; return $true
    } else { $g_ServersRDPFalse += $ComputerName ; Write-Host $false ; return $false }
    }
}
